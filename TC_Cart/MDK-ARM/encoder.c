...
float Target_Position_fixed_1, Target_Position_fixed_2;
...
{
    Target_Position_1 = Target_Position_1 + distance;
    Target_Position_2 = Target_Position_2 - distance;
}

void motor_turnback(float angle, float velocity)
{
    Target_Position_1 = Target_Position_1 - angle * 20;
    Target_Position_2 = Target_Position_2 + angle * 20;
}
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) // 定时器回调函数，用于计算速度
{
    ...
    if (htim->Instance == GAP_TIM.Instance) // 间隔定时器中断，是时候计算速度了
    {
        ...
        motor1.speed = 0.153 * (float)(motor1.totalCount - motor1.lastCount) / (4 * MOTOR_SPEED_RERATIO * PULSE_PRE_ROUND) * 3000; // 算得每秒多少转,除以4是因为4倍频
        /*******************在这里添加滤波函数************************/
        ...
        Now_Position_1=Now_Position_1*0.0136;
        if (pos_pid_flag)
        {
            // printf("position round");
            Target_Speed_fixed_1 = 0;
            Target_Speed_fixed_2 = 0;
            Target_Speed_1 = Location_PID_Realize(&pid_position_1, Target_Position_1 + Target_Position_fixed_1, Now_Position_1); // 位置环 Target_Position是目标位置，自行定义即可
        }
        motor1_Out = Speed_PID_Realize(&pid_speed_1, Target_Speed_1 + Target_Speed_fixed_1, motor1.speed); // 速度环
        ///////////////////////////////////////////////////////////////////////////////////////////
        ...
        motor2.speed = 0.153 * (float)(motor2.totalCount - motor2.lastCount) / (4 * MOTOR_SPEED_RERATIO * PULSE_PRE_ROUND) * 3000; // 算得每秒多少转,除以4是因为4倍频
        /*******************在这里添加滤波函数************************/
        ...
        Now_Position_2=Now_Position_2*0.0136;
        if (pos_pid_flag)
        {
            // printf("position round2");
            Target_Speed_fixed_1 = 0;
            Target_Speed_fixed_2 = 0;
            Target_Speed_2 = -Location_PID_Realize(&pid_position_2, Target_Position_2 + Target_Position_fixed_2, Now_Position_2); // 位置环 Target_Position是目标位置，自行定义即可
        }
        motor2_Out = Speed_PID_Realize(&pid_speed_2, Target_Speed_2 + Target_Speed_fixed_2, motor2.speed); // 速度环
        /*
        printf("%f,%f,%f,%f,%f,%f,%f,%f\n",

        motor1.speed, 
        Target_Speed_1, 
        Target_Position_1 + Target_Position_fixed_1, 
        Now_Position_1, 
        motor2.speed,
        Target_Speed_2, 
        Target_Position_2 + Target_Position_fixed_2, 
        Now_Position_2
        ...
        */
        ...
        if (motor1_Out >= 0)
        {
            ...
            // __HAL_TIM_SetCompare(&MOTOR1_TIM, MOTOR1_CHANNEL_FORWARD, motor1_Out);
            // __HAL_TIM_SetCompare(&MOTOR1_TIM, MOTOR1_CHANNEL_BACKWARD, 0);
        }
        else
        {
            ...
            // __HAL_TIM_SetCompare(&MOTOR1_TIM, MOTOR1_CHANNEL_BACKWARD, 0);
            // __HAL_TIM_SetCompare(&MOTOR1_TIM, MOTOR1_CHANNEL_FORWARD, fabs(motor1_Out));
        }
        // printf("speed1 = %f\r\n",motor1.speed);
        ...
        if (motor2_Out >= 0)
        {
            ...
            // __HAL_TIM_SetCompare(&MOTOR2_TIM, MOTOR2_CHANNEL_FORWARD, 0);
            // __HAL_TIM_SetCompare(&MOTOR2_TIM, MOTOR2_CHANNEL_BACKWARD, motor2_Out);
        }
        else
        {
            ...
            // __HAL_TIM_SetCompare(&MOTOR2_TIM, MOTOR2_CHANNEL_BACKWARD, 0);
            // __HAL_TIM_SetCompare(&MOTOR2_TIM, MOTOR2_CHANNEL_FORWARD, fabs(motor2_Out));
        }
        // printf("speed2 = %f\r\n",motor2.speed);
        // printf("%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n", motor1.speed, Target_Speed_1 + Target_Speed_fixed_1, motor2.speed, Target_Speed_2 + Target_Speed_fixed_2, Now_Position_1, Target_Position_1 + Target_Position_fixed_1, Now_Position_2, Target_Position_2 + Target_Position_fixed_2, motor1_Out, motor2_Out);
        // printf("%f,%f,%f,%f,%f,%f,%f,%f\n", motor1.speed, Target_Speed_1 + Target_Speed_fixed_1, motor2.speed, Target_Speed_2 + Target_Speed_fixed_2, motor1_Out, motor2_Out,pid_speed_1.integral,pid_speed_2.integral);
    }
}
