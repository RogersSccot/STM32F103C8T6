...
uint8_t receive_buff_2[50]; // 从树莓派（串�??1）接受的指令
...
/// @brief 串口1（树莓派或�?�电脑）信息处理
/// @param buffer 

// 移除字符串中指定字符的函数  
void removeChar(uint8_t *str) {  
...
void buffer_hadle_USART1(uint8_t* buffer)
{
  ...
  else
  {
    ...
    case 'L': // 左转-
      ...
      {
        left_angle = -(buffer[2] - '0') * 100 - (buffer[3] - '0');
      }
			Target_Position_1 -= left_angle*5.4;
			Target_Position_2 -= left_angle*5.4;
			last_order = 'L';
      break;
    ...
    case 'R': // 右转
      ...
      else if (buffer[1] == '-')
      {
        right_angle = -(buffer[2] - '0') * 10 - (buffer[3] - '0');
			...
			Target_Position_1 += right_angle*5.4;
			Target_Position_2 += right_angle*5.4;
			last_order = 'R';
      break;
    ...
    // 
      if (buffer[1] == '+')
      {
        ...
        Target_Speed_1 = velocity_set * 10;
        Target_Speed_2 = velocity_set * 10;
        Target_Speed_fixed_2 = 0;
        Target_Speed_fixed_1 = 0;
      }
      else if (buffer[1] == '-')
...
//			pos_pid_flag = 1;
//		}
//		printf("%.2f,%.2f,%.2f,%.4f,%.4f\n",angle_z_all,Target_Angle,Target_Speed_fixed_1,-pid_angle.ki*pid_angle.integral,-pid_angle.kp*pid_angle.err);
}
