...
uint8_t receive_buff[50]; // 从树莓派（串口1）接受的指令
uint8_t USART1_buffer[50];
uint8_t rx_buffer[50]; // 从陀螺仪（串口3）接受的角度数据
...
void buffer_hadle_USART1(uint8_t *buffer)
{
  ...
  else if ((buffer[0] == 'B') && (buffer[1] == 'E') && (buffer[2] == 'E') && (buffer[3] == 'P')) // GOOD
  {
    beep_on();
  }
  else if ((buffer[0] == 'Q') && (buffer[1] == 'U') && (buffer[2] == 'I') && (buffer[3] == 'T')) // GOOD
  {
    beep_off();
  }
  ...
  else
  {
    ...
    case 'E': // 纠正
      ...
      velocity_set = (buffer[7] - '0') * 100 + (buffer[8] - '0') * 10 + (buffer[9] - '0');
      last_order = 'E';
      // printf("E");
      break;
    ...
    case 'V': // 前进
      ...
      break;
      case 'P':
      KP_speed = (buffer[1] - '0') * 1.0 +  (buffer[3] - '0')*0.1+ (buffer[4] - '0')*0.01;
      PID_Init();
      break;
      case 'I':
      KI_speed = (buffer[1] - '0') * 1.0 +  (buffer[3] - '0')*0.1+ (buffer[4] - '0')*0.01;
      PID_Init();
      break;
      case 'D':
      KD_speed = (buffer[1] - '0') * 1.0 +  (buffer[3] - '0')*0.1+ (buffer[4] - '0')*0.01;
      PID_Init();
      break;
    default:
      break;
