...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) // 定时器回调函数，用于计算速度
{
    ...
    if (htim->Instance == GAP_TIM.Instance) // 间隔定时器中断，是时候计算速度了
    {
        ...
        motor1.speed = (float)(motor1.totalCount - motor1.lastCount) / (4.0 * MOTOR_SPEED_RERATIO * PULSE_PRE_ROUND) * 3000.0; // 算得每秒多少转,除以4是因为4倍频
        /*******************在这里添加滤波函数************************/
        ...
        Target_Speed_1 = velocity_set * 14.3;
         //Target_Speed_1 = Location_PID_Realize(&pid_position,Target_Position_1,Now_Position_1)/3;//位置环 Target_Position是目标位置，自行定义即可
        motor1_Out =Speed_PID_Realize(&pid_speed, Target_Speed_1 - Target_Speed_fixed_1, motor1.speed); // 速度环
        if (motor1_Out >= 0)
        {
        ...
        Target_Speed_fixed_2 = 0;//Line_PID_Realize(&pid_line,error);
        Target_Speed_2 = velocity_set * 14.3;
        motor2_Out = Speed_PID_Realize(&pid_speed, Target_Speed_2 - Target_Speed_fixed_2, motor2.speed); // 速度环
        if (motor2_Out >= 0)
        {
        ...
        //printf("%f,%f,%f,%f,%f\r\n",0.07*motor1.speed,0.07*motor2.speed,0.07*(Target_Speed_1-Target_Speed_fixed_1),0.07*(Target_Speed_2-Target_Speed_fixed_2),  10*error);
